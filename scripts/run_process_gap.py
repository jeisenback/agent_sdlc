"""Runner for the Process Gap Agent (business context check).
...
def _print_result(result: object, issue_number: int | None = None) -> None:
    ...
    print("\n" + "=" * 70)
    print(
        f"{label} Process Gap Check — {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}"
    )
    ...
    if not result.findings:
        print("  No findings — issue passes business process check.")
    print("=" * 70 + "\n")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the Process Gap Agent (business context check)."
    )
    parser.add_argument(
